...
test.describe('Dashboard - My Items', () => {
  ...
  test('should sort items by different columns', async ({ page }) => {
    ...
    const priceSortButton = page.locator('button:has-text("Precio")').first();
    if (await priceSortButton.count()) {
  ...
  test('should delete item from dashboard', async ({ page }) => {
    ...
    const deleteButton = row.locator('button[aria-label="Eliminar el item"]');
